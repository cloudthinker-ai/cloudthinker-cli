...
import { CT_CLOUD_READ, CT_CLOUD_WRITE, READ_TASK_OUTPUT } from "./names.ts";
import {
	type Elapsed,
	callComponent,
	callLine,
...
const parameters = Type.Object({
	...
	script: Type.String({
		description:
			...
			"CLI is already installed and authenticated. Bound the output yourself " +
			"(head, --max-items, jq) so a large listing does not fill the context.",
	}),
...
const description = [
	"Run a READ-ONLY command in CloudThinker's cloud Sandbox with a workspace Connection's credential injected.",
	...
	`Anything that changes cloud state (create, delete, apply, scale, restart, rotate, put, tag) belongs in ${CT_CLOUD_WRITE}, which the workspace approves.`,
].join("\n");
...
export function registerCloudRead(runtime: CloudThinkerRuntime): void {
	runtime.pi.registerTool<typeof parameters, ExecutionResult & Elapsed>({
		name: CT_CLOUD_READ,
		label: "Cloud read",
		description,
		promptSnippet:
			"Run a read-only command in CloudThinker's cloud with a workspace Connection's credential",
		promptGuidelines: [
			`Reach a workspace Connection only through ${CT_CLOUD_READ}. This machine holds no cloud credentials, so a local aws, kubectl, gcloud, or gh command cannot reach the user's cloud.`,
