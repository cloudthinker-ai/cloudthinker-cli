...
- The start-up offer speaks only when stdin and stderr are both terminals, so `assert_cmd` cannot reach it. `CA-UP-9` and `CA-UP-10` drive the real binary on a `portable-pty` pty with `CLOUDTHINKER_AGENT_BIN` pointed at a stub.
